...
private:
    ...
    void SendWrite(hidd_device* handle, std::vector<u8> buffer, int size);
    u8 GetCounter();
    std::vector<u8> SubCommand(hidd_device* handle, SubComamnd sc, std::vector<u8> buffer, int size);
    std::vector<u8> GetResponse(hidd_device* handle, SubComamnd sc);
    std::vector<u8> ReadSPI(hidd_device* handle, CalAddr addr, u8 size);
    ...
    bool CheckDeviceAccess(std::size_t port, hidd_device_info* device);
