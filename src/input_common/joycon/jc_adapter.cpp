...
void Joycons::SendWrite(hidd_device* handle, std::vector<u8> buffer, int size) {
    hidd_write(handle, buffer.data(), size);
}
...
std::vector<u8> Joycons::SubCommand(hidd_device* handle, SubComamnd sc, std::vector<u8> buffer,
                                    int size) {
...
std::vector<u8> Joycons::GetResponse(hidd_device* handle, SubComamnd sc) {
    int tries = 0;
    std::vector<u8> buffer(max_resp_size);
    do {
        int result = hidd_read_timeout(handle, buffer.data(), max_resp_size, 100);
        if (result < 1) {
...
std::vector<u8> Joycons::ReadSPI(hidd_device* handle, CalAddr addr, u8 size) {
...
void Joycons::GetUserCalibrationData(Joycon& jc) {
    ...
    hidd_set_nonblocking(jc.handle, 0);
    ...
    SetImuCal(jc, buffer);
    hidd_set_nonblocking(jc.handle, 1);
}
...
void Joycons::SetMac(Joycon& jc) {
    wchar_t mac[255];
    hidd_get_serial_number_string(jc.handle, mac, std::size(mac));
    for (int i = 0; i < 6; ++i) {
    ...
    while (adapter_thread_running) {
        for (std::size_t port = 0; port < joycon.size(); ++port) {
            if (joycon[port].type != JoyControllerTypes::None) {
                ...
                const int status =
                    hidd_read_timeout(joycon[port].handle, buffer.data(), max_resp_size, 15);
                if (status > 0) {
                    if (buffer[0] == 0x00) {
                        // Invalid response
...
void Joycons::Setup() {
    ...
    hidd_device_info* devs = hidd_enumerate(0x0, 0x0);
    hidd_device_info* cur_dev = devs;
...
bool Joycons::CheckDeviceAccess(std::size_t port, hidd_device_info* device) {
    ...
    joycon[port].handle = hidd_open(device->vendor_id, device->product_id, device->serial_number);
    if (!joycon[port].handle) {
        LOG_ERROR(Input, "Yuzu can not gain access to this device: ID {:04X}:{:04X}.",
                  device->vendor_id, device->product_id);
        return false;
    }
    hidd_set_nonblocking(joycon[port].handle, 1);
