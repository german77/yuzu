...
class TASMotion final : public Input::MotionDevice {
public:
    explicit TASMotion(u32 pad_, u32 motion_, const TasInput::Tas* tas_input_)
        : pad(pad_), motion(motion_), tas_input(tas_input_) {}

    Input::MotionStatus GetStatus() const override {
        return tas_input->GetTasState(pad).motions.at(motion);
    }
...
TasMotionFactory::TasMotionFactory(std::shared_ptr<TasInput::Tas> tas_input_)
    : tas_input(std::move(tas_input_)) {}
 ...
 */
std::unique_ptr<Input::MotionDevice> TasMotionFactory::Create(const Common::ParamPackage& params) {
    const auto pad = static_cast<u32>(params.Get("pad", 0));
    auto motion = static_cast<u32>(params.Get("motion", 0));

    return std::make_unique<TASMotion>(pad, motion, tas_input.get());
}
